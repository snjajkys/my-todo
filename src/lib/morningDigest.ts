 ...
 * 아침에 알려 줄 할 일인지 판단한다.
 *
 * 오늘 화면(`isVisibleOn`)의 미완료 규칙을 따르되, 한 가지가 다르다.
 * 아직 시작하지 않은 기간 할 일은 뺀다. 화면에서는 "시작까지 30일"로 남겨 두는 게
 * 도움이 되지만, 그걸 매일 아침 알림으로 받으면 한 달 내내 같은 알림이 온다.
 *
 * 그래서 두 종류 모두 "시작했고 아직 못 끝낸 것"이라는 한 가지 규칙으로 본다.
