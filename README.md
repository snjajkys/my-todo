...
| 항목 | 표시 조건 |
| --- | --- |
| 오늘 할 일 (미완료) | 기준 날짜부터 끝낼 때까지 **매일 계속 표시** (며칠 밀렸는지 함께 안내). 달력에서 앞날에 적어 둔 일은 그날이 되어야 올라옵니다 |
| 오늘 할 일 (완료) | **완료한 날에만** 표시 — 어제 끝낸 일이 오늘 목록을 채우지 않습니다 |
| 기간 할 일 (미완료) | 시작일부터 끝낼 때까지 **매일 계속 표시** (종료일이 지나면 며칠 지났는지 안내). 시작 전에는 오늘 목록에 오르지 않고 달력·주간 화면에서만 보입니다 |
| 기간 할 일 (완료) | **완료한 날에만** 표시 |
...
### 홈 화면에 "진짜 앱"으로 설치되게 하기

안드로이드에서 홈 화면에 추가하면 두 가지 중 하나가 만들어집니다.

| | 조건 | 결과 |
| --- | --- | --- |
| **WebAPK** (원하는 것) | 설치 조건을 모두 충족 | 독립된 앱으로 실행 |
| 바로가기 | 하나라도 미충족 | 크롬 안에서 실행 |

바로가기로 만들어지면 크롬이 **"탭하여 이 앱의 URL 복사하기"** 알림을 앱이 켜져
있는 내내 띄웁니다. 주소창이 없으니 어느 사이트인지 알려 주려는 것인데, 이 알림은
크롬 것이라 **끄면 우리 아침 알림까지 함께 막힙니다.** 그래서 알림을 끄는 것으로는
풀 수 없고, 설치 조건을 채우는 수밖에 없습니다.

충족해야 하는 것:

- HTTPS ✓
- 매니페스트에 `name`/`short_name`, `start_url`, `display: standalone` ✓
- **192px 과 512px 아이콘** — 512 하나만으로는 부족합니다 (`/icon-192`, `/icon`)
- **`fetch` 처리기가 있는 서비스 워커** — 비어 있는 처리기는 크롬이 없는 것으로 칩니다.
  `public/sw.js` 는 화면 이동만 가로채 오프라인 안내를 돌려줍니다. 캐싱은 하지 않습니다.
- 서비스 워커가 **어느 화면에서 들어와도 등록**되어야 합니다
  (`ServiceWorkerRegistrar` 를 루트 레이아웃에 둔 이유)

확인은 크롬에게 직접 물어볼 수 있습니다 — DevTools → Application → Manifest,
또는 CDP 의 `Page.getInstallabilityErrors`.

조건을 채운 뒤에도 **이미 만들어진 바로가기는 그대로입니다.** 홈 화면에서 지우고
다시 추가해야 WebAPK 로 다시 만들어집니다.

### 알림 아이콘이 두 가지인 이유

| 자리 | 그림 | 왜 |
| --- | --- | --- |
| `icon` (큰 아이콘) | `/icon` | 알림 안에 원래 색 그대로 보인다 |
| `badge` (작은 아이콘) | `/icon-badge` | **안드로이드가 알파 채널만 읽어 실루엣으로 쓴다** |

배경이 있는 앱 아이콘을 `badge` 에 주면 배경까지 실루엣이 되어 **검은 네모**가 뜹니다.
처음에 `/icon-maskable` 을 그대로 썼다가 실제로 그렇게 됐습니다. 그래서 `/icon-badge` 는
배경 없이 체크 표시만 그립니다. 다이어리 전체를 그리지 않는 것도 같은 이유입니다 —
상태 표시줄에서 24dp 안팎으로 작아지면 실루엣이 둥근 사각형 덩어리가 되어 또 네모로 보입니다.
...
> 이 기능을 만들며 공휴일 목록 사이트 두 곳을 확인했는데 서로 달랐고, 정작 키를 넣고
> 공식 데이터를 받아 보니 **양쪽 다 틀렸습니다.** 코드를 쓰던 사람(=이 주석을 처음 쓴
> Claude)까지 "제헌절은 2008년부터 공휴일이 아니다"라고 적어 두었다가 반증당했습니다 —
> 2026년 제헌절은 공휴일이고, 2027년에는 대체공휴일까지 붙습니다.
>
> 공휴일에 대해 "내가 아는 것"은 이렇게 조용히 낡습니다. 그래서 판단하지 않고 받아옵니다.
